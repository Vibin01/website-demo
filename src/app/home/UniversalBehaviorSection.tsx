    ...
    <div className="mt-lg p-md flex w-full sm:w-[75%] flex-col items-center justify-center rounded-md bg-[#E7F0FC]">
      <div className="flex flex-col sm:flex-row items-center gap-sm md:gap-lg">
        {flowTexts.map((text, index) => (
          <div key={text} className="flex flex-col sm:flex-row items-center gap-sm md:gap-lg">
            <span className={`${index !== flowTexts.length - 1 ? "font-medium":"font-bold"} text-base`}>{text}</span>

            {index !== flowTexts.length - 1 && (
              <span className="rotate-90 sm:rotate-0 "> <FaArrowRight className="size-iconsize-sm scale-90"/></span>
            )}
          </div>
        ))}
      </div>
